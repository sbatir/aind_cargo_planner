...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for actionNode in self.a_levels[level - 1]:
            #Get effectNode and add to stateNodes set. 
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return bool(
        #Does node 1's add get cancelled by node a2s remove? 
        set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)|
        #Does node 2's add get cancelled by a1s remove? 
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #Return TRUE if effect in node a1 is the negation of a precondition in a2. Perform for add and remove. 
        set(node_a1.action.effect_add) & set(node_a2.action.precond_neg) | set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos) |

        # Same as above, but reverse with a2 as reference to a1. 
        set(node_a2.action.effect_add) & set(node_a1.action.precond_neg) | set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos)
)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Competing Needs between Precondition? 
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        bothLiteralsArePos = node_s1.is_pos != node_s2.is_pos
        sameSign = node_s1.symbol == node_s2.symbol 

        theTruth = sameSign and bothLiteralsArePos
        return theTruth 
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        #each parent in s1 and s2 are the preconditions. 
        for s1Parent in node_s1.parents:
            for s2Parent in node_s2.parents:
                if not s1Parent.is_mutex(s2Parent):
                    return False
        #Logic is like competing needs but for states, not actions. 
        return True 
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            testNode = PgNode_s(goal, True) #Use property of PgNode "isPos" param. 
            listOfSLevels = enumerate(self.s_levels)
            for level, stateNodes in listOfSLevels:
                if testNode in stateNodes:
                    level_sum += level
                    break
        return level_sum
